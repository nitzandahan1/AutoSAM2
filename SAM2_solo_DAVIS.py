...
#VIDEO_DIR = r"C:\pythonProject\DL_project\dataset\DAVIS2017\DAVIS_train\JPEGImages\480p\bear"
#GT_DIR    = r"C:\pythonProject\DL_project\dataset\DAVIS2017\DAVIS_train\Annotations\480p\bear"
OUT_DIR   = r"./runs/val/naiv___24/"
...
def list_images_label(d, exts=(".png", ".jpg", ".jpeg", ".PNG", ".JPG", ".JPEG")) -> List[str]:
    ...
    if not names:
        raise FileNotFoundError(f"error {d}")
    try:
...
def sanity_check_first_frame(video_dir: str, video_dir_gt: str):
    ...
    print(f"found {len(frame_names)} images ו-{len(frame_names_gt)} masks.")
...
def select_largest_label_and_box_from_frame0(
    ...
    box_xyxy = box_xyxy.astype(np.int32).tolist()  # ללא נקודות עשרוניות
...
def save_eval_triptych(frame_rgb: np.ndarray,
    ...
    _prep_ax(ax1, 0.00, 0.3334)
    ax1.imshow(frame_rgb)
    ax1.imshow(pred_rgba)
    if draw_contour and pred.any():
        ax1.contour(pred.astype(np.uint8), levels=[0.5], linewidths=1.8)

    # GT
    ax2 = fig.add_subplot(1, 3, 2)
    _prep_ax(ax2, 0.3333, 0.3334)
    ax2.imshow(frame_rgb)
    ax2.imshow(gt_rgba)
    if draw_contour and gt.any():
        ax2.contour(gt.astype(np.uint8), levels=[0.5], linewidths=1.8)

    # Error map
    ax3 = fig.add_subplot(1, 3, 3)
    _prep_ax(ax3, 0.6666, 0.3334)
...
def evaluate_video(video_segments: dict,
                   frame_names: list,
                   frame_names_gt: list,
                   video_dir: str,
                   video_dir_gt: str,
    ...
    for curr_idx in range(n_frames):
        # --- GT ---
        gt_path = os.path.join(video_dir_gt, frame_names_gt[curr_idx])
        curr_gt = cv2.imread(gt_path, cv2.IMREAD_GRAYSCALE)
        if curr_gt is None:
            print(f"[warn] Could not load GT for frame {curr_idx} from {gt_path}. Skipping.")
            continue
        gt_mask_bool = (curr_gt > 0)
        H, W = gt_mask_bool.shape

        # --- Prediction ---
        if curr_idx not in video_segments or obj_id not in video_segments[curr_idx]:
            print(f"[warn] No predicted mask for frame {curr_idx} (obj {obj_id}). Skipping.")
            continue

        pred_raw = video_segments[curr_idx][obj_id]
        pred_bool = _to_bool_mask_2d(pred_raw, target_hw=(H, W))

        # --- Metrics (as in Net) ---
        dice, iou = _dice_and_iou_from_bool(pred_bool, gt_mask_bool)               # [KPI-NET]
        fscore    = _boundary_f_measure(pred_bool.astype(np.float32),
                                        gt_mask_bool.astype(np.float32),
                                        tol=tol_pixels)                            # [KPI-NET]
        jf        = 0.5 * (iou + fscore)                                           # [KPI-NET]
    ...
    if metrics_rows and write_csv_path is not None:
        import csv
        with open(write_csv_path, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=["frame", "dice", "iou", "f", "jf"])   # [KPI-NET]
            w.writeheader(); w.writerows(metrics_rows)

    if metrics_rows:
        mean_d = sum(r["dice"] for r in metrics_rows) / len(metrics_rows)           # [KPI-NET]
        mean_j = sum(r["iou"]  for r in metrics_rows) / len(metrics_rows)           # [KPI-NET]
        mean_f = sum(r["f"]    for r in metrics_rows) / len(metrics_rows)           # [KPI-NET]
        mean_jf= sum(r["jf"]   for r in metrics_rows) / len(metrics_rows)           # [KPI-NET]
        print(f"[metrics] {os.path.basename(video_dir)} | frames={len(metrics_rows)} | "
              f"Dice={mean_d:.4f} | J(IoU)={mean_j:.4f} | F={mean_f:.4f} | J&F={mean_jf:.4f}")  # [KPI-NET]
        return mean_j, mean_f, mean_jf, mean_d
    else:
        print("[metrics] No frames evaluated.")
        return None, None, None, None
...
if __name__ == "__main__":
    ...
    frame_names = list_images(VIDEO_DIR)
    frame_names_gt = list_images(GT_DIR)
    ...
    rgb0 = Image.open(os.path.join(VIDEO_DIR, frame_names[0]))
    W_rgb, H_rgb = rgb0.size  # PIL: (W,H)

    gt_masks_obj = load_gt_binary_masks_for_label(
        gt_dir=GT_DIR,
        frame_names_gt=frame_names_gt,
        label_id=label_id,
        target_size=(W_rgb, H_rgb),
    )

    print(f"[info] GT file: {gt_path}")
    print(f"[info] chosen label id: {label_id}")
    print(f"[info] box (xyxy): {box_xyxy}")
    print(f"[info] mask sum (pixels): {int(mask01.sum())}")
