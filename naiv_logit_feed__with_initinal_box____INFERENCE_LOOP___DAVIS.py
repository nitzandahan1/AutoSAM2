...
# ---- BBOX helpers ----
def _mask_to_box_xyxy(mask01: np.ndarray, pad_frac: float = 0.10, min_size: int = 16,
...
def _box_from_prior_logits(logits_1hw: np.ndarray,
                           prior_thr: float, pad_frac: float, min_box: int,
                           img_h: int, img_w: int):
    arr = logits_1hw[0] if logits_1hw.ndim == 3 else logits_1hw
    p = 1.0 / (1.0 + np.exp(-arr))
    prior_bin = (p >= prior_thr).astype(np.float32)
    return _mask_to_box_xyxy(prior_bin, pad_frac=pad_frac, min_size=min_box, H=img_h, W=img_w)
...
def _draw_box(img_bgr: np.ndarray, box_xyxy: Optional[Tuple[int,int,int,int]],
              color=(0,255,255), thickness=2) -> np.ndarray:
    out = img_bgr.copy()
...
def _save_panel(
    ...
    box_xyxy_t:  Optional[Tuple[int,int,int,int]] = None,  # בוקס לתמונה הנכנסת למודל
    is_t0:       bool = False
):
    os.makedirs(out_dir, exist_ok=True)
    ...
    img_t_bgr_vis = _draw_box(img_t_bgr, box_xyxy_t) if is_t0 else img_t_bgr

    prior_overlay = _mask_overlay(img_t_bgr, m_tm1_01, (255,0,0), 0.40) if m_tm1_01 is not None else _put_title(img_t_bgr_vis.copy(), "no prior (t=0)")
    pred_overlay  = _mask_overlay(img_t_bgr_vis, pred01,   (0,255,0), 0.45)

    row1_imgs = [
        _put_title(img_tm1_bgr, "I_{t-1}") if img_tm1_bgr is not None else _put_title(img_t_bgr_vis.copy(), "I_{t-1} (N/A)"),
        _put_title(img_t_bgr_vis,   "I_t (+BOX)" if is_t0 else "I_t"),
        _put_title(prior_overlay, "Prior (t=0)" if is_t0 else "Prior (from t-1)"),
        _put_title(pred_overlay,  "SAM2 pred (overlay)")
    ]
    row1 = _hstack_resize(row1_imgs, 300)

    if gt01 is not None:
        gt_overlay  = _mask_overlay(img_t_bgr_vis, gt01,   (0,0,255), 0.45)
...
def run_one_sequence_naive(
    ...
    for idx, (fn_img, fn_gt) in enumerate(zip(frame_files, mask_files)):
        ...
        if idx == 0:
            ...
            if BOX_FROM.lower() == "prior":
                box_xyxy = _box_from_prior_logits(
                    init_logits, PRIOR_THR, BOX_PAD_FRAC, MIN_BOX, H, W
                )
            else:
                box_xyxy = _mask_to_box_xyxy(gt_mask_bool.astype(np.float32),
                                             pad_frac=BOX_PAD_FRAC, min_size=MIN_BOX, H=H, W=W)

            t0_box_xyxy = box_xyxy

            if box_xyxy is None:
                print("[t=0] no box found (empty prior/gt); proceeding without box.")
            else:
                print(f"[t=0] box_xyxy={box_xyxy}")
        ...
        try:
            ...
            _save_panel(
                ...
                pred01=pred01,
                gt01=gt01,
                box_xyxy_t=(t0_box_xyxy if idx == 0 else None),
                is_t0=(idx == 0)
            )
...
if __name__ == "__main__":
    ...
    OUT_ROOT   = r".\runs\preds_val_davis_logit_box"
    LABEL_ID   = None
    MAX_VIZ_SEQS = None
