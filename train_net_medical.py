...
OUT_DIR = r".\runs\train_prompter_sam2\ALL_TRAIN_MASK_BOX__MEDICAL"
...
class MultiSeqDavisDataset(Dataset):
    def __init__(self, data_root: str, split_dir: str, res: str, seq_names: List[str]):
        ...
        for s_idx, name in enumerate(self.seq_names):
            ...
            assert len(frames) == len(masks) and len(frames) >= 2, f"בעיה עם {name}: מעט פריימים או אי-התאמה."
...
IMG_GLOB = "*_endo.png"
GT_GLOB_PRIMARY  = "*_endo_watershed_mask.png"
GT_GLOB_FALLBACK = "*_endo_mask.png"
PREFER_LABELS = (31, 32)
TRAIN_FRAC = 0.40
VAL_FRAC   = 0.20
TEST_FRAC  = 0.40
SPLIT_JSON_MEDICAL = os.path.join(OUT_DIR, "medical_split_40_20_40.json")

_frame_id_re = re.compile(r'(\d+)_endo(?:_|\.png)')

def _list_dirs(d: str) -> List[str]:
    return [p for p in sorted(glob.glob(os.path.join(d, "*"))) if os.path.isdir(p)]
...
def _pick_label_31_or_32(first_mask_path: str) -> Optional[int]:
    lm = load_label_map(first_mask_path)
    if (lm == 31).any(): return 31
    if (lm == 32).any(): return 32
    return None

def scan_medical_sequences(root_dir: str) -> Tuple[
    List[Tuple[str, List[str], List[str], int]],  # valid: (seq_dir, frames_sorted, masks_sorted, chosen_label)
    List[str]                                     # skipped (no 31/32 in first frame)
]:
    valid, skipped = [], []
    for vdir in _list_dirs(root_dir):          # video01, video12, ...
        for sdir in _list_dirs(vdir):          # video01_00160, ...
            img_paths = glob.glob(os.path.join(sdir, IMG_GLOB))
            msk_paths = glob.glob(os.path.join(sdir, GT_GLOB_PRIMARY))
            if len(msk_paths) == 0:
                msk_paths = glob.glob(os.path.join(sdir, GT_GLOB_FALLBACK))
            if len(img_paths) < 2 or len(msk_paths) < 2:
                continue

            imgs_by_id: Dict[int, str] = {}
            for p in img_paths:
                fid = _extract_frame_id(p)
                if fid is not None:
                    imgs_by_id[fid] = p

            msks_by_id: Dict[int, str] = {}
            for p in msk_paths:
                fid = _extract_frame_id(p)
                if fid is not None:
                    msks_by_id[fid] = p

            common_ids = sorted(set(imgs_by_id.keys()) & set(msks_by_id.keys()))
            if len(common_ids) < 2:
                continue

            first_id = common_ids[0]
            first_mask_path = msks_by_id[first_id]
            chosen = _pick_label_31_or_32(first_mask_path)
            if chosen is None:
                skipped.append(os.path.join(os.path.basename(os.path.dirname(sdir)), os.path.basename(sdir)))
                continue

            frames_sorted = [imgs_by_id[i] for i in common_ids]
            masks_sorted  = [msks_by_id[i] for i in common_ids]
            valid.append((sdir, frames_sorted, masks_sorted, chosen))
    return valid, skipped

class MedicalSeqDataset(Dataset):

    def __init__(self, packed: List[Tuple[str, List[str], List[str], int]]):
        assert len(packed) > 0, "No valid medical sequences with 31/32."
        self.seq_dirs   = [p[0] for p in packed]
        self.seq_frames = [p[1] for p in packed]
        self.seq_masks  = [p[2] for p in packed]
        self.seq_label  = [p[3] for p in packed]
        self.seq_names  = [os.path.basename(p) for p in self.seq_dirs]
        self.sample_index: List[Tuple[int,int]] = []
        self._printed_once = set()
        for s_idx, frames in enumerate(self.seq_frames):
            for t in range(1, len(frames)):
                self.sample_index.append((s_idx, t))
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        seq_idx, t = self.sample_index[idx]
        frames = self.seq_frames[seq_idx]
        masks  = self.seq_masks[seq_idx]
        label  = self.seq_label[seq_idx]
        ...
        lm_tm1 = load_label_map(masks[t-1])
        lm_t   = load_label_map(masks[t])
        if lm_tm1.shape != (H,W) or lm_t.shape != (H,W):
            raise RuntimeError(f"Image/GT size mismatch in {self.seq_dirs[seq_idx]}")

        m_tm1 = (lm_tm1 == label).astype(np.uint8)
        m_t   = (lm_t   == label).astype(np.uint8)

        if PRINT_GETITEM and t == 1:
            name = self.seq_names[seq_idx]
            if name not in self._printed_once:
                print(f"[getitem] {name}: label={label} present_tm1={m_tm1.any()} present_t={m_t.any()}")
                self._printed_once.add(name)
...
def build_or_load_medical_split(root: str,
                                json_path: str,
                                train_frac: float = TRAIN_FRAC,
                                val_frac: float   = VAL_FRAC,
                                seed: int = SEED) -> Dict[str, List[Tuple[str, List[str], List[str], int]]]:
    if os.path.isfile(json_path):
        with open(json_path, "r", encoding="utf-8") as f:
            meta = json.load(f)
        want = {k: set(meta[k]) for k in ("train","val","test")}
        packed_all, skipped = scan_medical_sequences(root)
        split = {"train": [], "val": [], "test": [], "_skipped": skipped}
        for item in packed_all:
            short = os.path.join(os.path.basename(os.path.dirname(item[0])), os.path.basename(item[0]))
            for k in ("train","val","test"):
                if short in want[k]:
                    split[k].append(item)
        if sum(len(v) for v in split.values() if isinstance(v, list)) == sum(len(v) for v in want.values()):
            return split

    packed, skipped = scan_medical_sequences(root)
    assert len(packed) >= 5, f"Need >=5 valid sequences with 31/32; found {len(packed)}."
    ...
    def short(sdir: str) -> str:
        return os.path.join(os.path.basename(os.path.dirname(sdir)), os.path.basename(sdir))

    json_out = {
        "train": [short(t[0]) for t in split["train"]],
        "val":   [short(t[0]) for t in split["val"]],
        "test":  [short(t[0]) for t in split["test"]],
        "seed":  seed
    }
...
# ----------------------------- main -----------------------------
def _nice_names(packed_list: List[Tuple[str, List[str], List[str], int]]) -> List[str]:
    return [os.path.join(os.path.basename(os.path.dirname(p[0])), os.path.basename(p[0])) for p in packed_list]
...
def main():
    ...
    if USE_MEDICAL:
        ...
        print(f"[info] scanning medical root: {MEDICAL_ROOT}")
        split = build_or_load_medical_split(MEDICAL_ROOT, SPLIT_JSON_MEDICAL, TRAIN_FRAC, VAL_FRAC, SEED)

        train_pack = split["train"]; val_pack = split["val"]; test_pack = split["test"]
        skipped    = split.get("_skipped", [])

        print("\n=== MEDICAL SPLIT (31/32 ONLY, 40/20/40) ===")
        print(f"TRAIN (n={len(train_pack)}): {_nice_names(train_pack)}")
        print(f"VAL   (n={len(val_pack)}):   {_nice_names(val_pack)}")
        print(f"TEST  (n={len(test_pack)}):  {_nice_names(test_pack)}")
        if skipped:
            print(f"SKIPPED (no 31/32 in first frame, n={len(skipped)}): {skipped}")
        print(f"Saved split JSON -> {SPLIT_JSON_MEDICAL}\n")

        ds_train = MedicalSeqDataset(train_pack)
        ds_val   = MedicalSeqDataset(val_pack)
        ...
        ds_train = MultiSeqDavisDataset(DATA_ROOT, "DAVIS_train", RES, train_seqs)
        ds_val   = MultiSeqDavisDataset(DATA_ROOT, "DAVIS_train", RES, val_seqs)
        loader_train = DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=True,
                                  num_workers=NUM_WORKERS, pin_memory=True,
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        for _step, batch in enumerate(pbar, start=1):
            ...
            with torch.amp.autocast(device_type="cuda", enabled=(DEVICE == "cuda")):
                ...
                for b in range(logits_t.shape[0]):
                    img_r = imgs_np[b]
                    sname = (loader_train.dataset.seq_names[int(batch["seq_idx"][b])])
        ...
        val_dice, val_iou = evaluate(prompter, predictor, loader_val, DEVICE, loader_val.dataset.seq_names, writer, epoch)
