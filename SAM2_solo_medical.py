...
# DAVIS (dual-dir):
VIDEO_DIR_DAVIS = r"C:\pythonProject\DL_project\dataset\DAVIS2017\DAVIS_train\JPEGImages\480p\bike-packing"
GT_DIR_DAVIS    = r"C:\pythonProject\DL_project\dataset\DAVIS2017\DAVIS_train\Annotations\480p\bike-packing"

# MEDICAL (single-dir: images + *watershed_mask*.png):
VIDEO_DIR_MED   = r"C:\pythonProject\DL_project\dataset\MEDICAL\video12\video12_19900"
OUT_DIR   = r"./runs/val/naiv___23_med/"
...
def list_images_davis(d, exts=(".jpg",".jpeg",".png",".JPG",".JPEG",".PNG")):
...
if __name__ == "__main__":
    if MEDICAL_MODE:
        VIDEO_DIR = VIDEO_DIR_MED
        frame_names = _sorted_medical_frames(VIDEO_DIR)
        if len(frame_names) < 2:
            raise RuntimeError(f"Need >=2 frames in {VIDEO_DIR}")
        ...
        box_xyxy = box_from_mask_label(mask01, pad_frac=0.05, min_box=16).astype(np.float32)
        ...
        if VIS:
            ...
            cv2.imwrite(os.path.join(OUT_DIR, f"rgb0_overlay_label_{label_id}.png"), cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR))
            ...
            fig.savefig(os.path.join(OUT_DIR, f"rgb0_with_box_id_{label_id}.png"),
                        dpi=160, transparent=CLEAN_VIZ, bbox_inches='tight', pad_inches=0)
            plt.close(fig)

        TMP_JPG_DIR = os.path.join(OUT_DIR, "sam2_jpg_seq")
        ...
        inference_state = predictor.init_state(video_path=TMP_JPG_DIR)  # <<< במקום VIDEO_DIR
        ...
        labels0 = read_davis_labels(os.path.join(GT_DIR, frame_names_gt[0]))
        uniq = [int(v) for v in np.unique(labels0) if int(v) > 0]
        if uniq:
            areas = [(lid, int((labels0==lid).sum())) for lid in uniq]
            label_id = max(areas, key=lambda t: t[1])[0]
        else:
            label_id = 1 if (labels0>0).sum()>0 else 0
        mask01  = (labels0 == label_id).astype(np.uint8)
        box_xyxy = box_from_mask_label(mask01, pad_frac=0.05, min_box=16).astype(np.int32).tolist()
        print(f"[info] DAVIS chosen label id: {label_id} | box={box_xyxy}")
